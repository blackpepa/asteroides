...
class Theme_Style_General extends The7_Tab_Base {
	...
	private function add_general_appearance_section() {
		...
		$this->add_control( 'the7_general_accessibility_outline_links', [
	...
	private function add_scroll_to_top_button_section() {
		...
		$this->start_controls_section( "{$name}", [
			'label' => __( 'Scroll To Top Button', 'the7mk2' ),
			'tab'   => $this->get_id(),
		] );

		$this->add_control( 'the7_scroll_to_top_button_enable', [
		...
		$this->add_responsive_control( "{$name}_icon_size", [
		...
		$this->add_responsive_control( "{$name}_size", [
		...
		$this->add_control( "{$name}_border_width", [
		...
		$this->add_control( "{$name}_border_radius", [
		...
		// Normal colors.
		$this->start_controls_tab( "{$name}_style_tab", [
		...
		// Hover colors.
		$this->start_controls_tab( "{$name}_style_tab_hover", [
		...
		$this->add_responsive_control( "{$name}_position", [
			'separator'            => 'before',
		...
		$this->add_responsive_control( "{$name}_h_offset", [
		...
		$this->add_responsive_control( "{$name}_v_offset", [
	...
	private function add_anchor_scroll_offset() {
		$this->start_controls_section( 'the7_anchor_scroll_settings', [
		...
		$this->add_responsive_control( "the7_anchor_scroll_offset", [
			'label'      => __( 'Offset for anchor navigation', 'the7mk2' ),
			'type'       => Controls_Manager::SLIDER,
			'size_units' => [ 'px'],
			'selectors'  => [
				$selector => '--the7-anchor-scroll-offset: {{SIZE}}',
			],
			'range'      => [
				'px' => [
					'min'  => -1000,
	...
	private function add_scroll_to_top_button_controls( $prefix = '' ) {
		...
		$this->add_control( 'the7_scroll_to_top_button_icon_color' . $prefix, [
		...
		$this->add_control( 'the7_scroll_to_top_button_bg_color' . $prefix, [
		...
		$this->add_control( 'the7_scroll_to_top_button_border_color' . $prefix, [
